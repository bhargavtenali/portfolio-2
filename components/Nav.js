//icons
import { HiHome, HiUser, HiChatBubbleBottomCenterText } from "react-icons/hi2";
...
export const navData = [
  ...
  { name: "Feedback", path: "/feedback", icon: <MdDesignServices /> },
        ...
        {navData.map((link, index) => {
          return (
            <Link
              className={`${
                link.path === pathname &&
                " text-blue-100 shadow-md border-am border-sky-200 shadow-black p-2 border-2 rounded-full bg-gradient-to-r from-sky-800 via-sky-600 to-sky-400"
              } text-2xl bg  relative flex items-center group hover:scale-[1.1] transition-all duration-100`}
              href={link.path}
              key={index}
            >
              {/* tooltip */}
